...
async function createApplicationWindow() {
  ...
  if (!fs.existsSync(path.join(__dirname, "node_modules", "sharp"))) {
    ...
    ApplicationLogger.getInstance().info(`Added \`sharp\` module to global paths.`);
  }
...
app.on("ready", async () => {
  try {
    ...
    ApplicationLogger.getInstance().error(error, "Failed to initialize the application window.");
    app.quit();
  }
});
